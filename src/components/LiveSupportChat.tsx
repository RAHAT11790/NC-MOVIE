...
const LiveSupportChat = ({ getAnimeList, isOpen, onClose, onAnimeSelect }: LiveSupportChatProps) => {
  ...
  useEffect(() => {
    ...
    const buildContext = () => {
      let ctx = "";
      if (appUserData) {
        ctx += `यूजर नाम: ${appUserData.name || "अज्ञात"}\n`;
        ctx += `ईमेल: ${appUserData.email || commaKey.replace(/,/g, ".")}\n`;
        ctx += `पासवर्ड सेट है: ${appUserData.password ? "हाँ" : "नहीं"}\n`;
        if (appUserData.password) {
          ctx += `वर्तमान पासवर्ड: ${appUserData.password}\n`;
        }
        ctx += `लॉगिन विधि: ${appUserData.googleUid ? "Google" : "Email/Password"}\n`;
      }
      if (premiumData) {
        ctx += `प्रीमियम स्थिति: ${premiumData.active ? "सक्रिय ✅" : "निष्क्रिय ❌"}\n`;
        if (premiumData.active) {
          ...
          if (expiry) {
            ...
            ctx += `प्रीमियम अवधि: ${remaining} दिन शेष (${expiry.toLocaleDateString("bn-BD")})\n`;
          }
          if (premiumData.deviceLimit) ctx += `डिवाइस लिमिट: ${premiumData.deviceLimit}टी\n`;
          if (premiumData.devices) {
            const deviceCount = typeof premiumData.devices === "object" ? Object.keys(premiumData.devices).length : 0;
            ctx += `सक्रिय डिवाइस: ${deviceCount}\n`;
          }
        }
      }
      setUserContext(ctx);
  ...
  const buildAnimeContext = useCallback(() => {
    ...
    context += `RS Catalog (${primaryItems.length}टी):\n`;
  ...
  const sendMessage = async () => {
    ...
    const asksPassword = hasAny(["password", "pass", "पासवर्ड", "पस", "id pass", "आईडी पस"]);
    const asksEmail = hasAny(["email", "gmail", "mail", "ईमेल", "id", "आईडी"]);
    const asksPremium = hasAny(["premium", "प्रीमियम", "subscription", "सब्सक्रिप्शन", "अवधि", "दिन बाकी", "device", "डिवाइस", "limit", "सीमा"]);
    const asksAccountInfo = asksPassword || asksEmail || asksPremium;
    const asksAnime = hasAny(["anime", "एनिमे", "एनिमे", "movie", "फिल्म", "series", "सीरीज़", "episode", "एपिसोड", "देख", "दो", "link", "लिंक", "दो", "suggest", "recommend", "naruto", "one piece", "dragon ball", "attack on titan", "demon slayer"]);
    const isGreeting = hasAny(["hi", "hello", "hey", "assalamu alaikum", "अस्सलाम अलैकुम", "अस्सलाम अलैकुम", "सलाम", "हैलो", "हाय"]);
    const isTinyMessage = normalizedText.split(" ").filter(Boolean).length <= 3;
    const buildLocalReply = () => {
      if (isGreeting && isTinyMessage) {
        return `अस्सलाम अलैकुम ${userName || "भाई"}! 👋\nमैं RS AI हूँ। Anime, episode, premium, ID/password—जो जानना चाहते हैं, लिखें।`;
      }

      if (!userContext) return "";

      const name = extractField("यूजर नाम") || "आप";
      const email = extractField("ईमेल");
      const password = extractField("वर्तमान पासवर्ड");
      const premiumStatus = extractField("प्रीमियम स्टेटस");
      const premiumExpiry = extractField("प्रीमियम अवधि");
      const deviceLimit = extractField("डिवाइस सीमा");
      const activeDevices = extractField("सक्रिय डिवाइस");

      if (asksPassword && asksEmail) {
        return `${name}, आपकी लॉगिन जानकारी:\n• ईमेल/आईडी: ${email || "नहीं मिला"}\n• पासवर्ड: ${password || "सेट नहीं किया गया"} 🔐`;
      }
      if (asksPassword) {
        return password ? `${name}, आपका वर्तमान पासवर्ड: ${password} 🔐` : `${name}, आपके खाते में कोई पासवर्ड सेट नहीं किया गया है।`;
      }
      if (asksEmail) {
        return email ? `${name}, आपका लॉगिन ईमेल/आईडी: ${email} 📩` : `${name}, आपका लॉगिन ईमेल/आईडी नहीं मिला।`;
      }
      if (asksPremium) {
        const info = [
        ...
        return info.length ? `${name}, आपकी प्रीमियम जानकारी:\n${info.join("\n")} ✨` : "";
      }
      return "";
    };
    ...
    if (text.includes("@RS") || text.includes("@rs") || text.includes("@Rs")) {
      ...
      try {
        ...
        const adminReply: ChatMessage = { id: `a_${Date.now()}`, role: "assistant", content: "✅ आपका मैसेज Admin को भेजा गया है! Admin जवाब देगा तो यहीं देखेंगे। 😊", timestamp: Date.now() };
        setMessages(prev => [...prev, adminReply]);
      } catch {
        toast.error("मैसेज भेजने में विफल रहा");
      }
      return;
    }
    ...
    try {
      ...
      if (!aiConfig?.enabled || !aiConfig?.url) {
        throw new Error("AI configured नहीं है");
      }
      ...
      if (!res.ok && !aiReply) {
        throw new Error(data?.error || `AI त्रुटि ${res.status}`);
      }
      if (!aiReply) {
        throw new Error("खाली AI जवाब");
      }
      ...
      const errMsg: ChatMessage = {
        ...
        role: "assistant",
        content: isRateLimit
          ? "⏳ अभी बहुत ज़्यादा मैसेज आ रहे हैं। कृपया 15 सेकंड बाद फिर से कोशिश करें।"
          : "⚠️ सर्वर में समस्या हो रही है। कृपया थोड़ी देर बाद फिर से प्रयास करें। सीधे Admin तक पहुंचने के लिए @RS लिखकर मैसेज करें।",
        timestamp: Date.now(),
      };
  ...
  return (
    <div className="fixed left-2 right-2 sm:left-auto sm:right-3 sm:w-[370px] z-[60] rounded-2xl overflow-hidden flex flex-col bg-background"
      ...
      <div className="p-3" style={{ boxShadow: "0 -2px 8px rgba(0,0,0,0.04)" }}>
        ...
        <p className="text-[9px] text-muted-foreground text-center mt-2">@RS लिखकर एडमिन को सीधे मैसेज करें</p>
      </div>
    </div>
  );
};
