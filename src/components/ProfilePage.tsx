          ...
          <>
            ...
            {(paymentTab === "bkash" && hasBkash) ? (
              <div>
                ...
                {selectedPlan && (
                  <div className="premium-card p-4 rounded-2xl mb-4">
                    <h4 className="text-sm font-semibold mb-3 text-[#E2136E]">📲 पेमेंट करें</h4>
                    
                    {/* bKash Info */}
                    <div className="bg-[#E2136E]/10 border border-[#E2136E]/30 rounded-xl p-3 mb-3">
                      <p className="text-xs text-muted-foreground mb-1">{bkashSettings.accountType} नहींंबर:</p>
                      <p className="text-lg font-bold text-[#E2136E] tracking-wider">{bkashSettings.phoneNumber}</p>
                      <p className="text-xs text-muted-foreground mt-1">मात्रा: <span className="font-bold text-foreground">₹{selectedPlan.price}</span></p>
                    </div>
                    ...
                    {bkashSettings.qrCodeLink && (
                      <div className="text-center mb-3">
                        <p className="text-xs text-muted-foreground mb-2">QR कोड स्कैन करें:</p>
                    ...
                    {/* bKash Sender Number */}
                    <div className="mb-3">
                      <label className="text-xs text-muted-foreground mb-1 block">आपका bKash नंबर</label>
                    ...
                    <div className="mb-3">
                      ...
                      <input value={trxInput} onChange={e => setTrxInput(e.target.value.toUpperCase())}
                        placeholder="जैसे: TrxID 0A1B2C3D4E"
                    ...
                    <button onClick={submitBkashPayment} disabled={trxSubmitting || !trxInput.trim() || !bkashSenderNumber.trim()}
                      ...
                      {trxSubmitting ? "सबमिट हो रहा है..." : "✅ सबमिट करें"}
                    </button>
                  </div>
                )}
              </div>
    ...
    return (
      <motion.div className="fixed inset-0 z-[200] bg-background overflow-y-auto pt-[70px] px-4 pb-24"
        ...
        <button onClick={() => setActivePanel("change-password")}
          ...
          <Lock className="w-4 h-4 text-primary" /> पासवर्ड सेटिंग्स
        </button>
      </motion.div>
    );
  }
      ...
      {deviceExceeded && deviceCheckDone && (
        <div className="mb-4 rounded-xl border border-destructive/40 bg-destructive/10 p-3">
          <div className="flex items-start gap-2.5">
            ...
            <div className="flex-1">
              <p className="text-xs font-semibold text-foreground">आपकी आईडी इस फोन में लॉग इन है, लेकिन यह फोन प्रीमियम प्ले के लिए अधिकृत नहीं है।</p>
              <p className="text-[11px] text-muted-foreground mt-1">आप चाहें तो नीचे दिए गए बटन से इस फोन से आईडी डिलीट करके ऑटो लॉगआउट कर सकते हैं।</p>
              <button
                onClick={handleDeleteThisPhoneLogin}
      ...
      <div className="flex flex-col gap-2">
        <div onClick={() => setActivePanel("premium")}
          ...
          <div className="flex-1">
            ...
            {!isPremium && <p className="text-[10px] text-muted-foreground">bKash से प्रीमियम खरीदें</p>}
          </div>
...
const ChangePasswordPanel = ({ onBack }: { onBack: () => void }) => {
  ...
  const handleSetOrChangePassword = async () => {
    if (hasExistingPassword) {
      ...
      if (!oldPassword.trim() || !newPassword.trim()) { toast.error("सभी फ़ील्ड भरें"); return; }
      if (newPassword.length < 4) { toast.error("नया पासवर्ड कम से कम 4 अक्षरों का होना चाहिए"); return; }
      if (oldPassword === newPassword) { toast.error("नया पासवर्ड पुराने जैसा नहीं हो सकता"); return; }
      if (newPassword !== confirmPassword) { toast.error("पासवर्ड मेल नहीं खा रहा!"); return; }
    } else {
      // Set password flow - no old password needed
      if (!newPassword.trim()) { toast.error("पासवर्ड दर्ज करें"); return; }
      if (newPassword.length < 4) { toast.error("पासवर्ड कम से कम 4 अक्षरों का होना चाहिए"); return; }
      if (newPassword !== confirmPassword) { toast.error("पासवर्ड मेल नहीं खा रहा!"); return; }
    }
    ...
    try {
      ...
      if (hasExistingPassword) {
        if (userData.password !== oldPassword) {
          toast.error("पुराना पासवर्ड गलत है!");
      ...
      toast.success(hasExistingPassword ? "पासवर्ड बदल गया है! ✅" : "पासवर्ड सेट हो गया! ✅");
    ...
    <motion.div className="fixed inset-0 z-[200] bg-background overflow-y-auto pt-[70px] px-4 pb-24"
      ...
      <div className="glass-card p-5 rounded-2xl mb-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-sm font-bold">{hasExistingPassword ? "पासवर्ड बदलें" : "पासवर्ड सेट करें"}</h3>
            <p className="text-[10px] text-muted-foreground">
              {hasExistingPassword ? "पुराने पासवर्ड से नया पासवर्ड सेट करें" : "आपके खाते में अभी तक कोई पासवर्ड सेट नहीं किया गया है"}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {hasExistingPassword && (
            <div>
              <label className="text-xs text-muted-foreground mb-2 block">पुराना पासवर्ड</label>
              <div className="relative">
          ...
          <div>
            <label className="text-xs text-muted-foreground mb-2 block">{hasExistingPassword ? "नया पासवर्ड" : "पासवर्ड"}</label>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          ...
          <div>
            <label className="text-xs text-muted-foreground mb-2 block">पासवर्ड की पुष्टि करें</label>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          ...
          {newPassword && confirmPassword && newPassword === confirmPassword && (
            <p className="text-xs text-green-500 flex items-center gap-1.5"><Check className="w-3.5 h-3.5" /> पासवर्ड मिल रहा है</p>
          )}
          {newPassword && confirmPassword && newPassword !== confirmPassword && (
            <p className="text-xs text-destructive">पासवर्ड मेल नहीं खा रहा!</p>
          )}
        </div>
      </div>
      ...
      <button onClick={handleSetOrChangePassword} disabled={loading}
        ...
        {loading ? <span className="animate-spin w-4 h-4 border-2 border-white/30 border-t-white rounded-full" /> : <><Save className="w-4 h-4" /> {hasExistingPassword ? "पासवर्ड बदलें" : "पासवर्ड सेट करें"}</>}
      </button>
    ...
    if (storageKey === "rs_notif_push" && next) {
      try {
        ...
        if ("Notification" in window && Notification.permission === "default") {
          ...
          if (permission !== "granted") {
            toast.error("अधिसूचना अनुमति नहीं दी गई है। ब्राउज़र सेटिंग्स से अनुमति दें।");
            return; // Don't toggle on if not granted
          }
        } else if ("Notification" in window && Notification.permission === "denied") {
          toast.error("❌ नोटिफिकेशन ब्लॉक किए गए हैं! ब्राउज़र सेटिंग्स → नोटिफिकेशन → अनुमति दें।");
          return;
        }
        setEnabled(next);
      ...
      if (storageKey === "rs_notif_push" && u?.id) {
        try {
          ...
          toast.error("Push disable sync विफल");
        }
      }
    }
  };
