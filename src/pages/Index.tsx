...
const Index = () => {
  ...
  const checkAndShowAdGate = useCallback(async (): Promise<boolean> => {
    ...
    if (!isLoggedIn) {
      toast.error("वीडियो देखने के लिए लॉगिन करना होगा");
      return false;
    }

    if (unlockBlocked) {
      toast.error("एक ही अनलॉक टोकन के दुरुपयोग के कारण यह अकाउंट ब्लॉक कर दिया गया है");
      return false;
    }
    ...
    try {
      ...
      if (result.ok && result.shortUrl) {
        ...
        toast.error("Unlock link नहीं बनाया जा सका, फिर से कोशिश करें");
      }
    } catch {
      setSaltAdGateLoading(false);
      setSaltAdGateActive(false);
      toast.error("Unlock service अभी अनुपलब्ध है");
    }
    return false;
            ...
            if (snap.val() === true) {
              ...
              if (!sessionStorage.getItem(sessionKey)) {
                ...
                if (permission === "granted") {
                  await registerFCMToken(userId, false);
                  toast.success("✅ नोटिफिकेशन चालू हो गया है!", { duration: 3000 });
                }
              }
            }
          } catch {}
  ...
  const handlePlay = async (anime: AnimeItem, seasonIdx?: number, epIdx?: number) => {
    if (unlockBlocked) {
      toast.error("यह अकाउंट टोकन के गलत इस्तेमाल के कारण वीडियो एक्सेस ब्लॉक किया गया है");
      return;
    }
  ...
  const handleContinueWatching = async (item: any) => {
    if (unlockBlocked) {
      toast.error("यह अकाउंट token मिसयूज़ के कारण वीडियो एक्सेस ब्लॉक है");
      return;
    }
